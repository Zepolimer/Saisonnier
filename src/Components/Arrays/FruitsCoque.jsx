...
import {FruitCoqueData} from '../../Data/FruitsCoqueData';
import Aside from '../Modal/Aside';

// ---------- CONTAINER
const FruitCoqueContainer = styled.section`
  width: 90%;
  margin: 0 auto;
  display: flex;
...
const FruitCoqueMapped = styled.button`
...
// ---------- NOM DU FRUIT
const FruitCoqueName = styled.span`
...
export default function FruitsCoque({ monthSaison }) {
  ...
  return (
    <FruitCoqueContainer>
      {FruitCoqueData.map((item, index) => {
        let itemSaison = item.saison;

        if(itemSaison && itemSaison.includes(monthSaison)){
          return (
          <FruitCoqueMapped key={index} onClick={() => {
              setmodalAside(true); 
              setThisIndex(item);
            }}>
            <img src={item.image} alt={item.name} />
            <FruitCoqueName>{item.name}</FruitCoqueName>
          </FruitCoqueMapped>
          )
        }
      })}
